...
conth<-95 # cylindrical container/pond height (cm)
...
continit<-95 # initial container water level (cm)
...
contwet<-0 # percent wet value for container
...
datestart<-'02/02/2009' # choose starting date (day, month, year)
datefinish<-'16/02/2009'# choose ending date (day, month, year)

# code relating to date selection - no need to change
tzone<-paste("Etc/GMT-10",sep="") # doing it this way ignores daylight savings!
datest<-strsplit(datestart,'/')[[1]]
datefn<-strsplit(datefinish,'/')[[1]]
ds<-ISOdate(as.numeric(datest[3]),as.numeric(datest[2]),as.numeric(datest[1]),tz=tzone)-3600*12
df<-ISOdate(as.numeric(datefn[3]),as.numeric(datefn[2]),as.numeric(datefn[1]),tz=tzone)-3600*12+3600*24
ndays<-as.numeric(df-ds)
dates<-seq(ISOdate(as.numeric(datest[3]),as.numeric(datest[2]),as.numeric(datest[1]),tz=tzone)-3600*12, ISOdate(as.numeric(datefn[3]),as.numeric(datefn[2]),as.numeric(datefn[1]),tz=tzone)-3600*13+3600*24, by="hours") 
dates2<-seq(ISOdate(as.numeric(datest[3]),as.numeric(datest[2]),as.numeric(datest[1]),tz=tzone)-3600*12, ISOdate(as.numeric(datefn[3]),as.numeric(datefn[2]),as.numeric(datefn[1]),tz=tzone)-3600*13+3600*24, by="days") 
time_offset<-0
dates<-unique(dates)
dates<-subset(dates, format(dates, "%m/%d")!= "02/29") # remove leap years
dates2<-subset(dates, format(dates2, "%m/%d")!= "02/29") # remove leap years
daystart<-paste(substr(datest[3],3,4),'/',datest[2],'/',datest[1],sep="") # y/m/d
dayfin<-paste(substr(datefn[3],3,4),'/',datefn[2],'/',datefn[1],sep="") # y/m/d
plotenviron<-subset(environ, format(as.POSIXlt(environ$dates), "%y/%m/%d")>=daystart & format(as.POSIXlt(environ$dates), "%y/%m/%d")<=dayfin)
plotmetout<-subset(metout, format(as.POSIXlt(metout$dates), "%y/%m/%d")>=daystart & format(as.POSIXlt(metout$dates), "%y/%m/%d")<=dayfin)


FTG_2009<-read.csv(paste(microin,'/Ferntree Gully Feb 2009.csv',sep=""))
FTG_2009$DateTime<-as.POSIXct(FTG_2009$Date,tz="Etc/GMT-10",format="%d/%m/%Y %H:%M")-3600 # format the date column



with(plotenviron,plot(WATERTEMP~dates,type='l',xlab='time (days)',ylim=c(0,50))) #use the plot command for the very first graph
#with(Tbs,points(Tskin~dates,type='l',xlab='time (days)',ylim=c(0,40),lty=2)) #now use the 'points' command to add skin temp onto the original plot, but with a dashed line (lty=2)
#points(Tairf(time)~dates,type='l',col='blue')
#with(FTG_2008,{points(Wheelie.Centre.Sun~DateTime,type = "l",col='red')})
with(FTG_2009,{points(Wheelie~DateTime,type = "l",col='red')})


with(plotmetout,plot(TS~dates,type='l',xlab='time (days)',ylim=c(0,70),col='light blue')) #use the plot command for the very first graph
with(FTG_2009,{points(Cup1~DateTime,type = "l",col='red')})
...
datestart<-'07/09/2008' # choose starting date (day, month, year)
datefinish<-'25/09/2008'# choose ending date (day, month, year)
...
#with(FTG_2008,{points(Wheelie.Centre.Sun~DateTime,type = "l",col='red')})
with(FTG_2008,{points(Wheelie.Centre.Sun~DateTime,type = "l",col='red')})
#with(plotmetout,points(TAREF~dates,type='l',xlab='time (days)',ylim=c(0,70),col='light blue')) #use the plot command for the very first graph

with(plotmetout,plot(TS~dates,type='l',xlab='time (days)',ylim=c(0,70))) #use the plot command for the very first graph
with(FTG_2008,{points(Substrate.Sun~DateTime,type = "l",col='red')})
