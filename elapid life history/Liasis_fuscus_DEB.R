...
TMAXPR<-33.5 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Fig. 4 Shine, R. and T. Madsen (1996). "Is thermoregulation unimportant for most reptiles? An example using water pythons (Liasis fuscus) in tropical Australia." Physiological Zoology 69(2): 252-269.
TMINPR<-28 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Fig. 4 Shine, R. and T. Madsen (1996). "Is thermoregulation unimportant for most reptiles? An example using water pythons (Liasis fuscus) in tropical Australia." Physiological Zoology 69(2): 252-269.
TBASK<-28 # degrees C, minimum basking temperature # Fig. 4 Shine, R. and T. Madsen (1996). "Is thermoregulation unimportant for most reptiles? An example using water pythons (Liasis fuscus) in tropical Australia." Physiological Zoology 69(2): 252-269.
TEMERGE<-21 # degrees C, temperature at which animal will move to a basking site # Fig. 4 Shine, R. and T. Madsen (1996). "Is thermoregulation unimportant for most reptiles? An example using water pythons (Liasis fuscus) in tropical Australia." Physiological Zoology 69(2): 252-269.
ctmax<-41.7  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded) # Heatwole 1981b cited in Greer 1997
ctmin<-20 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing) Lillywhite, H. B. (1980). "Behavioral Thermoregulation in Australian Elapid Snakes." Copeia 1980(3): 452-458.
...
TPREF<-30 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # from thermal gradient, Shine, R. and T. Madsen (1996). "Is thermoregulation unimportant for most reptiles? An example using water pythons (Liasis fuscus) in tropical Australia." Physiological Zoology 69(2): 252-269.
...
shdburrow<-1 #
mindepth<-6 # minimum depth (soil node) to which animal can retreat if burrowing
...
amass<-2000 # g, mass of animal (used if the 'monthly' option is checked and DEB model is thus off)
MR_1<-0.013
MR_2<-0.8
MR_3<-0.038
...
eggdryfrac<-0.5 # decimal percent, dry mass of eggs
...
dates<-seq(ISOdate((ystart+ystrt),1,1,tz=tzone)-3600*12, ISOdate((ystart+ystrt+nyears),1,1,tz=tzone)-3600*13, by="hours")
...
dates2<-seq(ISOdate(ystart+ystrt,1,1,tz=tzone)-3600*12, ISOdate((ystart+ystrt+nyears),1,1,tz=tzone)-3600*13, by="days") 
...
with(bask,points((TIME-1)~JULDAY,pch=15,cex=2,col='yellow'))
with(forage,points((TIME-1)~JULDAY,pch=15,cex=2,col='grey'))
plotenviron<-subset(environ, as.numeric(environ$dates,'%d-%m-%Y')>=757432800 & as.numeric(environ$dates,'%d-%m-%Y')<=757432800+3600*7*24)
