...
minshade<-80. # minimum available shade (percent)
...
TMAXPR<-34 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
TMINPR<-21.0 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Kearney Obs (PhD field trip)
TBASK<-21.0 # degrees C, minimum basking temperature (14. deg C, Fraser 1985 thesis, min of A in Fig. 7.3)
TEMERGE<-21.0 # degrees C, temperature at which animal will move to a basking site
...
TPREF<-28. # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
gutfill<-5. # % gut fill at which satiation occurs - if greater than 100%, animal always tries to forage
...
climb<-1 # climbing to seek cooler habitats allowed (1) or not (0)?
...
E_Hb<-41320*fract^3
E_Hj<-E_Hb*fract^3
E_Hp<-4.93e05*fract^3
h_aref<-0.0000000166/(24.^2) #3.61e-11/(24.^2) 
s_G<-0.01

E_Egg<-114000*fract^3# J, initial energy of one egg # this includes the residual yolk, which is eaten upon hatching
...
eggmass<-4.697 # initial dry mass of an egg (g)
...
plot(debout$WETMASS~debout$dates,type='l')
points(debout$WETMASS_STD~debout$dates,type='l',col=2)
plot(debout$SVL/10~debout$dates,type='l')
...
Tb<-19.4# put your guess in here and then run the next line to see what TC that implies
...
Tb<-22.8 # put your guess in here and then run the next line to see what TC that implies
exp(TA*(1/(273+T_REF)-1/(273+Tb)))/(1+exp(TAL*(1/(273+Tb)-1/TL))+exp(TAH*(1/TH-1/(273+Tb))))
