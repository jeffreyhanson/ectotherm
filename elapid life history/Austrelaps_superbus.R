...
TMAXPR<-37.1 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
TMINPR<-20.0 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Kearney Obs (PhD field trip)
TBASK<-15.0 # degrees C, minimum basking temperature (14. deg C, Fraser 1985 thesis, min of A in Fig. 7.3)
...
TPREF<-32. # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
Tb<-27.1 # put your guess in here and then run the next line to see what TC that implies
...
Tb<-27 # put your guess in here and then run the next line to see what TC that implies
exp(TA*(1/(273+T_REF)-1/(273+Tb)))/(1+exp(TAL*(1/(273+Tb)-1/TL))+exp(TAH*(1/TH-1/(273+Tb))))