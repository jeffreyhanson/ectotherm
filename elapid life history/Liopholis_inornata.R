...
TMAXPR<-33.6 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Elia thermal gradient average of minima
TMINPR<-24.6 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Elia thermal gradient average of minima
TBASK<-21.0 # degrees C, minimum basking temperature, Elia Weather room data
TEMERGE<-12 # degrees C, temperature at which animal will move to a basking site # Elia Weather room data, CTmin is 8.5 and they emerged to bask at 15 deg C, so took a value in between
ctmax<-43.  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded)
ctmin<-8.6 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing)
...
TPREF<-29.8 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Elia thermal gradient, all animals from hydrated round
...
maxdepth<-7 # maximum depth (soil node) to which animal can retreat if burrowing, 30 cm max depth based on Pianka, E. R. and W. F. Giles (1982). "Notes on the biology of two species of nocturnal skinks, Egernia inornata and Egernia striata, in the Great Victorian Desert." Western Austrailan Naturalist 15: 8-13.
...
amass<-150. # g, mass of animal (used if the 'monthly' option is checked and DEB model is thus off)
MR_1<-0.013
MR_2<-0.8
MR_3<-0.038
...
TH<-273+40
TC<-environ$TC
summer<-subset(environ, format(dates, "%m")<= "12" & format(dates, "%m")>= "10") # remove leap years
TCsumm<-summer$TC
TC<-as.numeric(exp(TA*(1/(273+T_REF)-1/(273+TC)))/(1+exp(TAL*(1/(273+TC)-1/TL))+exp(TAH*(1/TH-1/(273+TC)))))
plot(TC~dates)
mean(TC)

Tb<-30 # put your guess in here and then run the next line to see what TC that implies
...
Tb<-31.4 # put your guess in here and then run the next line to see what TC that implies
exp(TA*(1/(273+T_REF)-1/(273+Tb)))/(1+exp(TAL*(1/(273+Tb)-1/TL))+exp(TAH*(1/TH-1/(273+Tb))))
