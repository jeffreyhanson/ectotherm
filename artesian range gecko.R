...
TMAXPR<-32. # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection)
TMINPR<-25. # degrees C, voluntary thermal minimum (lower body temperature for foraging)
...
ctmax<-32.  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded)
...
TPREF<-30 # preferred body temperature (animal will attempt to regulate as close to this value as possible)
...
plotenviron_forage<-subset(environ,ACT==2)
with(plotenviron_forage, {plot(TIME~DAY,ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=1,pch=16)})
write.csv(environ,'environ.csv')
