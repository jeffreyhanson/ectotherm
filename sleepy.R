...
plotgrass<-subset(grass,format(grass$dates, "%y")>0)
#with(plotgrass,plot(growth~dates,type='l',col='dark green'))


rainfall<-as.data.frame(cbind(dates2,rainfall))
colnames(rainfall)<-c('dates','rainfall')
plotrainfall <- subset(rainfall,format(dates, "%y")>0)
#with(plotrainfall,points(rainfall~dates,type='l',col='blue'))
...
with(debout, {xyplot(WETMASS~dates,type = "l",ylab = "wet mass (g)/grass growth",groups=Stage,auto.key=list(columns = 5))})

with(debout, {plot((WETMASS-WETMASS*(Body_cond/100))~dates,type = "l",xlab = "day of year",ylab = "wet mass (g)")})
with(debout, {points(WETMASS~dates,type = "l",ylab = "wet mass (g)")})


# with(plotgrass, {points(growth*50~dates,type='l',col='dark green')})
with(rainfall, {points(rainfall*10~dates,type='h',col='dark blue')})
#  with(environ, {points(CONDEP*30~dates,type='l',col='light blue')})

with(environ, {points(CONDEP*10~dates,type='l',col='light blue')})
TAREF<-metout$TAREF
environ<-cbind(environ,TAREF)

curyear<-2009
daystart<-paste(substr(curyear,3,4),'/09/01',sep="") # y/m/d
dayfin<-paste(substr(curyear,3,4),'/09/31',sep="") # y/m/d
...
with(plotenviron_all, {points(CONDEP/10~JULDAY, type = "h",col='blue')})
...
plotrainfall2 <- subset(plotrainfall2,format(dates, "%y")=="10")
with(plotrainfall2, {points(plotrainfall2~doy2,type='h',col='dark blue')})
...
massdata<-read.csv('C:/NicheMapR_Working/projects/Sleepy Lizards/Waddle/Church_site_mass_length.csv')
...
par(bty="l")
boxplot(massagg_merge$mass.x,massagg_merge$mass.y,xlab="Time",ylab="body mass (g)",names=c("desiccated","hydrated"),col=c("lightblue","lightgreen"),ylim=c(400,1000))
...
for(i in 1:121){
  ...
  if(curyear==yeardo){
    ...
    #   
    filename<-paste("c:/NicheMapR_Working/projects/Sleepy Lizards/plots/",sleepy_id,"_",sexliz$Sex,"_",curyear,"_nodrink.pdf",sep="") 
    ...
    if(curyear==2009){
      plotenvironyear<-subset(plotenviron,YEAR==2)
    }else{
      plotenvironyear<-subset(plotenviron,YEAR==3)
    }
    with(plotenvironyear, {points(CONDEP/10~JULDAY, type = "l",col='dark green')})
    ...
    with(plotenviron, {points(CONDEP/10~dates, type = "l",col='dark green')})
    
    #with(plotgrass, {points(growth~dates, type = "l",col='dark green')})
    with(plotdeb,{plot((WETMASS-WETMASS*(Body_cond/100))~dates,type='l',ylim=c(400,1000))})
...
if(yeardo==2009){
  if(raindrink==0){
    write.csv(summary,'/NicheMapR_Working/projects/Sleepy Lizards/summary_drink_bask_2009.csv')
  }else{
    write.csv(summary,'/NicheMapR_Working/projects/Sleepy Lizards/summary_nodrink_2009.csv')
  }
}else{
  if(raindrink==0){
    write.csv(summary,'/NicheMapR_Working/projects/Sleepy Lizards/summary_drink_nobask_2010.csv')
  }else{
    write.csv(summary,'/NicheMapR_Working/projects/Sleepy Lizards/summary_nodrink_2010.csv')
  }    
}   
...
for(i in 1:121){
  ...
  if(curyear==yeardo){
    ...
    desic<-as.data.frame(cbind(desic[,2],desic[,20]))
    colnames(desic)<-c('day','desic')
    sleepy<- subset(sleepy,  subset=(Steps>threshold.act))
    date_waddle<-aggregate(sleepy$date_waddle,by=list(paste(sleepy$Year,"_",sleepy$Month,"_",sleepy$Day,"_",sleepy$Hours,sep="")),min)
    doy<-aggregate(sleepy$doy,by=list(paste(sleepy$Year,"_",sleepy$Month,"_",sleepy$Day,"_",sleepy$Hours,sep="")),max)
    Tb<-aggregate(sleepy$Temperature,by=list(paste(sleepy$Year,"_",sleepy$Month,"_",sleepy$Day,"_",sleepy$Hours,sep="")),mean)
    steps<-aggregate(sleepy$Steps,by=list(paste(sleepy$Year,"_",sleepy$Month,"_",sleepy$Day,"_",sleepy$Hours,sep="")),sum)
    ...
    sleepy$date_waddle<-as.POSIXct(as.numeric(date_waddle[,2]+9.5*60*60),tz=tzone,origin="1970-01-01")
    plotlizard_forage <- subset(sleepy,  subset=(Steps>0))
    plotlizard_noforage <- subset(sleepy,  subset=(Steps>=0))
    if(k==1){
      allforage<-cbind(plotlizard_forage,sexlizard)
      allnoforage<-plotlizard_noforage
...
Tbs$Col<-rbPal(23)[as.numeric(cut(Tbs$Tb, breaks=seq(18, 37,1), include.lowest=TRUE))]
plotenviron_bask$Col<-Tbs[1:nrow(plotenviron_bask),2]
plotenviron_for<-subset(plotenviron_bask,TC>=23.1)
#plotenviron_bask$Col <- rbPal(10)[as.numeric(cut(plotenviron_bask$TC,breaks = 10))]
#with(plotenviron_bask, {points(TIME~JULDAY, xlim=c(startdy,finishdy),ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=1.,col=plotenviron_bask$Col,pch=15)})
with(plotenviron_for, {points(TIME~JULDAY, xlim=c(startdy,finishdy),ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=1.,col=plotenviron_bask$Col,pch=15)})
...
with(meanwaddle, {points(Hours+1~doy, xlim=c(startdy,finishdy),ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=log(meanwaddle$Steps)/10,pch=15,col=addTrans("red",100))}) #col="#0000FF96" #0000FFFF
...
waddle2<-meanwaddle
#waddle2<-subset(waddle2,Steps>300)

with(waddle2, {points(Hours+1~doy, xlim=c(startdy,finishdy),ylim=c(0,25),xlab = "day of year",ylab = "hour of day",pch=15,col=addTrans("red",100))}) #col="#0000FF96" #0000FFFF
...
with(plotpred, {plot(TC~dates,type = "l",ylim=c(0,45))})
#with(plotmetout, {points(TAREF~dates,type = "l",col='grey')})
...
with(plotdeb,{plot((WETMASS-WETMASS*(Body_cond/100))~dates,type='l',ylim=c(500,800),ylab='wet mass, g')})
...
for(i in 1:121){
  ...
  if(curyear==yeardo){
    ...
    c.act<-as.data.frame(correl$ACT)
    c.shade<-as.data.frame(correl$SHADE)
    correl2<-cbind(sleepy_id,sexlizard,correl$dates,c.doy,c.year,c.month,c.day,c.hour,c.Tb_obs,c.steps,c.Tb_pred,c.act,c.shade)
    colnames(correl2)<-c('sleepy_id','sexlizard','date','doy','year','month','day','hour','Tb_obs','steps','Tb_pred','act','shade')
    ...
    c.act.day<-as.data.frame(correl_day$ACT)
    c.shade.day<-as.data.frame(correl_day$SHADE)
    correl2.day<-cbind(sleepy_id,sexlizard,correl_day$dates,c.doy.day,c.year.day,c.month.day,c.day.day,c.hour.day,c.Tb_obs.day,c.steps.day,c.Tb_pred.day,c.act.day,c.shade.day)
    colnames(correl2.day)<-c('sleepy_id','sexlizard','date','doy','year','month','day','hour','Tb_obs','steps','Tb_pred','act','shade')
...
library(aqfig)
with(correl_all,scatterplot.density(Tb_obs,Tb_pred,ylim=c(0,50),xlim=c(0,50),xlab='Tb observed',ylab='Tb predicted',col.regression.line=2, col.one.to.one.line=1))

correl_females<-subset(correl_all,sexlizard=='F')
correl_males<-subset(correl_all,sexlizard=='M')
correl_nov<-subset(correl_all,month==9)
with(correl_nov,scatterplot.density(Tb_obs,Tb_pred,ylim=c(0,50),xlim=c(0,50),xlab='Tb observed',ylab='Tb predicted',col.regression.line=2, col.one.to.one.line=1))

with(correl_all,scatterplot.density(Tb_pred,Tb_obs,ylim=c(0,50),xlim=c(0,50),xlab='Tb predicted',ylab='Tb observed',col.regression.line=2, col.one.to.one.line=1))
...
lm_Tb<-with(correl_all_2010,(lm(Tb_obs~Tb_pred)))

lm_Tb_R2<-summary(lm_Tb)$r.squared
lm_Tb_rmsd<-sqrt(mean(((correl_all_2010$Tb_obs-correl_all_2010$Tb_pred)^2),na.rm=TRUE))

thresh<-50
daystart<-paste('10/09/01',sep="") # y/m/d
dayfin<-paste('10/10/31',sep="") # y/m/d
correl_sub<-subset(correl_all,format(correl_all$date, "%y/%m/%d")>= daystart & format(correl_all$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "p",cex=0.2,col='red',ylim=c(0,50))})
with(correl_sub, {points(Tb_pred~date,type = "p",cex=0.5,ylim=c(0,40),pch=16)}) 

correl_all_mean1<-aggregate(correl_all,by=list(correl_all$date),mean)           
with(correl_all_mean1,scatterplot.density(Tb_obs,Tb_pred,ylim=c(0,50),xlim=c(0,50),xlab='Tb observed',ylab='Tb predicted',col.regression.line=2, col.one.to.one.line=1))
correl_all_mean_2010<-correl_all_mean


correl_sub<-subset(correl_all_mean1,format(correl_all_mean1$date, "%y/%m/%d")>= daystart & format(correl_all_mean1$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "l",cex=0.2,col='red',ylim=c(0,40))})
with(correl_sub, {points(Tb_pred~date,type = "p",cex=0.3,ylim=c(0,40),pch=16)}) 

lm_Tb<-with(correl_all_mean_2010,(lm(Tb_obs~Tb_pred)))

lm_Tb_R2<-summary(lm_Tb)$r.squared
lm_Tb_rmsd<-sqrt(mean(((correl_all_mean_2010$Tb_obs-correl_all_mean_2010$Tb_pred)^2),na.rm=TRUE))

lm_Tb<-with(correl_sub,(lm(Tb_obs~Tb_pred)))
...
lm_Tb_R2<-summary(lm_Tb)$r.squared
lm_Tb_rmsd<-sqrt(mean(((correl_all_mean_2010$Tb_obs-correl_all_mean_2010$Tb_pred)^2),na.rm=TRUE))
correl_all_mean<-correl_all_mean1
correl_all_mean$steps[correl_all_mean$steps<thresh]<-0
correl_all_mean$steps[correl_all_mean$steps>thresh]<-1
correl_all_mean$act[correl_all_mean$act==2]<-1
correl_all_mean$act[correl_all_mean$act==0]<--0.5
correl_all_mean$steps[correl_all_mean$steps==1]<-40
correl_all_mean$act[correl_all_mean$act==1]<-39.5


daystart<-paste('09/10/19',sep="") # y/m/d
dayfin<-paste('09/10/31',sep="") # y/m/d
correl_sub<-subset(correl_all_mean,format(correl_all_mean$date, "%y/%m/%d")>= daystart & format(correl_all_mean$date, "%y/%m/%d")<=dayfin)
with(correl_sub, {plot(Tb_obs~date,type = "l",pty=2,col='red',ylim=c(0,40))})
with(correl_sub, {points(Tb_pred~date,type = "l",ylim=c(0,40))})
#  with(correl_sub, {points(act~date,type = "p",ylim=c(0,40),cex=.5,pch=15)})
#  with(correl_sub, {points(steps~date,type = "p",pty=2,col='red',cex=.5,pch=15,ylim=c(0,40))})


daystart<-paste('09/09/01',sep="") # y/m/d
dayfin<-paste('09/09/30',sep="") # y/m/d
correl_sub<-subset(correl_all_mean,format(correl_all_mean$date, "%y/%m/%d")>= daystart & format(correl_all_mean$date, "%y/%m/%d")<=dayfin)
with(correl_sub, {plot(Tb_pred~date,type = "l",ylim=c(0,40))})
with(correl_sub, {points(Tb_obs~date,type = "l",pty=2,col='red',ylim=c(0,40))})
with(correl_sub, {points(act~date,type = "p",ylim=c(0,40),cex=.5,pch=15)})
with(correl_sub, {points(steps~date,type = "p",pty=2,col='red',cex=.5,pch=15,ylim=c(0,40))})

daystart<-paste('09/10/01',sep="") # y/m/d
dayfin<-paste('09/10/31',sep="") # y/m/d
correl_sub<-subset(correl_all_mean,format(correl_all_mean$date, "%y/%m/%d")>= daystart & format(correl_all_mean$date, "%y/%m/%d")<=dayfin)
with(correl_sub, {plot(Tb_pred~date,type = "l",ylim=c(0,40))})
with(correl_sub, {points(Tb_obs~date,type = "l",pty=2,col='red',ylim=c(0,40))})
...
daystart<-paste('09/11/01',sep="") # y/m/d
dayfin<-paste('09/11/30',sep="") # y/m/d
correl_sub<-subset(correl_all_mean,format(correl_all_mean$date, "%y/%m/%d")>= daystart & format(correl_all_mean$date, "%y/%m/%d")<=dayfin)
with(correl_sub, {plot(Tb_pred~date,type = "l",ylim=c(0,40))})
with(correl_sub, {points(Tb_obs~date,type = "l",pty=2,col='red',ylim=c(0,40))})
with(correl_sub, {points(act~date,type = "p",ylim=c(0,40),cex=.5,pch=15)})
...
daystart<-paste('09/12/01',sep="") # y/m/d
dayfin<-paste('09/12/31',sep="") # y/m/d
correl_sub<-subset(correl_all_mean,format(correl_all_mean$date, "%y/%m/%d")>= daystart & format(correl_all_mean$date, "%y/%m/%d")<=dayfin)
with(correl_sub, {plot(Tb_pred~date,type = "l",ylim=c(0,40))})
with(correl_sub, {points(Tb_obs~date,type = "l",pty=2,col='red',ylim=c(0,40))})
with(correl_sub, {points(act~date,type = "p",ylim=c(0,40),cex=.5,pch=15)})
...
daystart<-paste('09/08/26',sep="") # y/m/d
dayfin<-paste('09/09/30',sep="") # y/m/d
correl_sub<-subset(correl_all,format(correl_all$date, "%y/%m/%d")>= daystart & format(correl_all$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "p",cex=0.2,col='red',ylim=c(0,50),ylab='Tb')})
with(correl_sub, {points(Tb_pred~date,type = "p",cex=0.5,ylim=c(0,40),pch=16)}) 
...
daystart<-paste('10/08/26',sep="") # y/m/d
dayfin<-paste('10/09/30',sep="") # y/m/d
correl_sub<-subset(correl_all,format(correl_all$date, "%y/%m/%d")>= daystart & format(correl_all$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "p",cex=0.2,col='red',ylim=c(0,50),ylab='Tb')})
with(correl_sub, {points(Tb_pred~date,type = "p",cex=0.5,ylim=c(0,40),pch=16)}) 

daystart<-paste('10/10/01',sep="") # y/m/d
dayfin<-paste('10/10/31',sep="") # y/m/d
correl_sub<-subset(correl_all,format(correl_all$date, "%y/%m/%d")>= daystart & format(correl_all$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "p",cex=0.2,col='red',ylim=c(0,50),ylab='Tb')})
...
daystart<-paste('10/11/01',sep="") # y/m/d
dayfin<-paste('10/11/30',sep="") # y/m/d
correl_sub<-subset(correl_all,format(correl_all$date, "%y/%m/%d")>= daystart & format(correl_all$date, "%y/%m/%d")<=dayfin)
correl_sub<-correl_sub[order(correl_sub$date),]
with(correl_sub, {plot(Tb_obs~date,type = "p",cex=0.2,col='red',ylim=c(0,50),ylab='Tb')})
with(correl_sub, {points(Tb_pred~date,type = "p",cex=0.5,ylim=c(0,40),pch=16)}) 
...
daystart<-paste('10/12/01',sep="") # y/m/d
