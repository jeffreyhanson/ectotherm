      FUNCTION ZBRENTwing(funwing,X1,X2,TOL)
C     EPS IS MACHINE FLOATING POINT PRECISION
C     FLOATING POINT PRECISION FOR THE COMPAQ III PORTABLE IS 5.0E-20
      REAL A,B,C,D,E,DIAGNOS,EPS,FA,FB,FC,funwing
      REAL P,Q,R,S,TOL,TOL1,X1,X2,XM,ZBRENTwing
      INTEGER ITER,ITMAX
...
c        WRITE (0,*)'ROOT MUST BE BRACKETED FOR zbrentwing.' 
       ELSE
      ENDIF  
      FC=FB
      DO 11 ITER=1,ITMAX
...
c        WRITE(0,*) 'zbrentwing EXCEEDING MAXIMUM ITERATIONS.' 
      ENDIF  
      zbrentwing=B
20    RETURN
      END
