...
TMAXPR<-34.4 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
TMINPR<-26.8 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Kearney Obs (PhD field trip)
TBASK<-TMINPR#5 # degrees C, minimum basking temperature (14. deg C, Fraser 1985 thesis, min of A in Fig. 7.3)
TEMERGE<-15.27 # degrees C, temperature at which animal will move to a basking site
ctmax<-41.78  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded)
ctmin<-4.27 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing)
...
TPREF<-32.74 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
E_Hj<-E_Hb*fract^3
E_Hp<-7659*fract^3
h_aref<-3.293e-09/(24.^2) #3.61e-11/(24.^2) 
s_G<-0.01

E_Egg<-7.001E+03*fract^3# J, initial energy of one egg # this includes the residual yolk, which is eaten upon hatching
...
E_H_init<-0
stage<-0
v_init<-(0.7147^3)*fract^3 #hatchling
E_init<-E_m
E_H_init<-E_Hb+5
stage<-1
# v_init<-(1.688^3)*fract^3*0.85
# E_init<-E_m
# E_H_init<-E_Hp+1
# stage<-3
...
plot(debout$SVL~debout$dates,type='l')
...
TC<-environ$TC
  ...
  Tb<-18.3# put your guess in here and then run the next line to see what TC that implies
