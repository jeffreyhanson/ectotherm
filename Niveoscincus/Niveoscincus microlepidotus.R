...
TPREF<-30 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
E_Hj<-E_Hb*fract^3
E_Hp<-3441*fract^3
h_aref<-3.466e-08/(24.^2) #3.61e-11/(24.^2) 
s_G<-0.01

E_Egg<-3695*fract^3# J, initial energy of one egg # this includes the residual yolk, which is eaten upon hatching
...
startday<-30*10 # make it 90 for T. rugosa loop day of year at which DEB model starts
...
E_H_init<-0
stage<-0
v_init<-(0.6016^3)*fract^3 #hatchling
E_init<-E_m
E_H_init<-E_Hb+5
stage<-1
...
with(bask,points((TIME-1)~JULDAY,pch=15,cex=.5,col='blue'))
with(forage,points((TIME-1)~JULDAY,pch=15,cex=.5,col='grey'))
...
TC<-environ$TC
  ...
  Tb<-13.3# put your guess in here and then run the next line to see what TC that implies
