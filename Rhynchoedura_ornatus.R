...
if(curyear==2009){
  if(raindrink==0){
    write.csv(summary,'sleepy/summary_drink_2009.csv')
  }else{
    write.csv(summary,'sleepy/summary_nodrink_2009.csv')
  }
}else{
  if(raindrink==0){
    write.csv(summary,'sleepy/summary_drink_2010.csv')
  }else{
    write.csv(summary,'sleepy/summary_nodrink_2010.csv')
  }    
}   
...
library(aqfig)
with(correl_all,scatterplot.density(Tb_obs,Tb_pred,ylim=c(0,50),xlim=c(0,40),xlab='Tb observed',ylab='Tb predicted',col.regression.line=2, col.one.to.one.line=1))
...
lm_Tb<-with(correl_all_2010,(lm(Tb_pred~Tb_obs)))
