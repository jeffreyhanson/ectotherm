...
TMAXPR<-40. # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
...
TPREF<-31. # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
subdate<-subset(environ, format(environ$dates,"%y/%m")=="91/02")
...
species<-"Lucasium_stenodactylum"
data<-pitgrid_raw
a<-(pitgrid_raw$Species.Code==species)==TRUE
a[a==TRUE]<-1
a[a==FALSE]<-0
data<-cbind(pitgrid_raw,a)
data$dates<-as.POSIXct(as.Date(data$Date,"%d/%m/%Y"))+3600*6
data<-data[,18:19]
...
plotenviron2 <-subset(plotenviron1, format(plotenviron$dates, "%Y")==yeartoplot)
...
with(plotenviron_night, {plot(TIME~DAY, ylim=c(0,24),xlab = "day of year",ylab = "hour of day",cex=0.5,col=addTrans("dark grey",50),pch=16,main=i)})
with(plotenviron_trap, {points((TIME-1)~DAY, ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=0.5,col='black',pch=15)})
with(plotenviron_presence, {points((TIME-1)~DAY, ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=0.5,col='blue',pch=15)})
with(plotenviron_absence, {points((TIME-1)~DAY, ylim=c(0,25),xlab = "day of year",ylab = "hour of day",cex=0.5,col='red',pch=15)})
