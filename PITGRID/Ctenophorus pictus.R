...
TMAXPR<-38.0 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # Licht 1966 thermal gradient
TMINPR<-30 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # Kearney Obs (PhD field trip)
TBASK<-30 # degrees C, minimum basking temperature (14. deg C, Fraser 1985 thesis, min of A in Fig. 7.3)
...
TPREF<-34.4 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # Licht 1966 thermal gradient
...
with(rainfall,points(rainfall~dates
...
plotenviron2 <-subset(plotenviron1, format(plotenviron$dates, "%Y/%m")==paste(yeartoplot,"/02",sep=""))
...
plotenviron_night<-subset(plotenviron_night,format(plotenviron_night$dates, "%Y/%m")==paste(yeartoplot,"/02",sep=""))
