...
timeinterval<-365 # number of time intervals computed in a year (min of 12, i.e. monthly, max of 365, make the same as was set for the microclimate run)
nyears<-20 # number of years the simulation runs for 
...
ctmax<-43.0 # degrees C, critical thermal maximum (used by program to determine depth selected when inactive and burrowing)
...
yr<-1
plot_forage<-subset(environ,ACT==2 & YEAR==yr)
plot_bask<-subset(environ,ACT==1 & YEAR==yr)
plot_night<-subset(metout,ZEN<90)
with(plot_night,plot(TIME/60~JULDAY,pch=15,cex=2,col='yellow',ylim=c(0,24)))
with(plot_forage,points((TIME-1)~JULDAY,pch=15,cex=2,col='grey'))
with(plot_bask,points((TIME-1)~JULDAY,pch=15,cex=2,col='blue'))
